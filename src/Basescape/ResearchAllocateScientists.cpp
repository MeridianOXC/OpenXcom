	...
	: _base(base), _planningProject(planningProject), _otherCraftColor(0), _origSoldierOrder(*_base->getSoldiers()), _dynGetter(NULL)
{
	...
	_btnOk->onKeyboardPress((ActionHandler)&ResearchAllocateScientists::btnDeassignCraftSoldiersClick, Options::keyRemoveSoldiersFromCraft);
...
#undef PUSH_IN
	...
	_lstScientists->onMouseClick((ActionHandler)&ResearchAllocateScientists::lstSoldiersClick, 0);
}
 ...
 * @param action Pointer to an action.
 */
void ResearchAllocateScientists::lstSoldiersClick(Action *action)
{
 ...
 * @param action Pointer to an action.
 */
void ResearchAllocateScientists::btnDeassignCraftSoldiersClick(Action *action)
{
