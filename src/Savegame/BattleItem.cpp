 ...
 * @param groundOffset The number of inventory units the ground container is offset from 0, if any.
 * @return A rectangle that describes the inventory sprite's bounds within it's container.
*/
SDL_Rect BattleItem::getInvSpriteBounds(int groundOffset) const
{
	...
	case INV_GROUND:
		...
		break;
	default:
		throw std::logic_error("Item: " + _rules->getType() + " in inventory with bad enum value for slotType: " + std::to_string(_inventorySlot->getType()));
	}
...
/**
 * @brief Gets the items inventory sprite bounds relative to a handslot container, assuming that it is to be centered.
 * @return A rectangle that describes the inventory sprite's bounds within a hand-slot sized container.
*/
SDL_Rect BattleItem::getHandCenteredSpriteBounds() const
{
	// item dimensions in inventory units.
	int invSlotW = _rules->getInventoryWidth();
	int invSlotH = _rules->getInventoryHeight();
